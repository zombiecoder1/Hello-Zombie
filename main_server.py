...
class ChatRequest(BaseModel):
    agent: Optional[str] = Field(default="hello_zombie", description="Agent identifier")
    input: Optional[str] = Field(default=None, min_length=1, max_length=10000, description="User input")
    message: Optional[str] = Field(default=None, min_length=1, max_length=10000, description="User message (alternative to input)")
    context: Optional[Dict[str, Any]] = Field(default=None, description="Additional context")
    
    def __init__(self, **data):
        # Handle both 'input' and 'message' fields
        if 'message' in data and 'input' not in data:
            data['input'] = data['message']
        elif 'input' not in data and 'message' not in data:
            raise ValueError("Either 'input' or 'message' field is required")
        super().__init__(**data)
...
def generate_system_prompt() -> str:
    ...
    try:
        ...
        prompt_file = Path("config/agent_prompts/zombiecoder_system_prompt.txt")
...
async def get_providers_status():
    """Get providers status for dashboard compatibility"""
    try:
        ollama_healthy = await check_ollama_health()
