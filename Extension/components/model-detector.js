...
function ModelDetector({ onModelChange, onStatusUpdate }) {
    ...
    const [autoDetect, setAutoDetect] = (0, react_1.useState)(true);
    ...
    const detectModels = (0, react_1.useCallback)(async (modelList) => {
        ...
        for (const model of modelsToScan) {
            ...
            const isAvailable = model.type === "local"
        ...
        setIsScanning(false);
        setScanProgress(100);
        const availableModels = modelsToScan.filter((m) => models.find((model) => model.id === m.id)?.status === "available");
    ...
    const loadModel = (0, react_1.useCallback)(async (modelId) => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            default:
                return <lucide_react_1.AlertTriangle className="h-4 w-4 text-gray-400"/>;
        }
    };
    const getProviderIcon = (provider) => {
        switch (provider.toLowerCase()) {
            case "zombiecoder":
                return <lucide_react_1.Brain className="h-4 w-4 text-purple-400"/>;
            case "ollama":
                return <lucide_react_1.Server className="h-4 w-4 text-blue-400"/>;
            case "lm studio":
                return <lucide_react_1.Cpu className="h-4 w-4 text-green-400"/>;
            case "openai":
            case "anthropic":
                return <lucide_react_1.Globe className="h-4 w-4 text-orange-400"/>;
            default:
                return <lucide_react_1.Database className="h-4 w-4 text-gray-400"/>;
        }
    };
    ...
    return (<card_1.Card className="bg-slate-800 border-slate-700">
      <card_1.CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <button_1.Button size="sm" onClick={() => detectModels()} disabled={isScanning} className="h-7 bg-green-600 hover:bg-green-700">
      ...
      <card_1.CardContent className="space-y-4">
        {/* Auto Detect Toggle */}
        <div className="flex items-center justify-between">
          <label_1.Label className="text-sm text-gray-300">Auto Detect:</label_1.Label>
          <switch_1.Switch checked={autoDetect} onCheckedChange={setAutoDetect}/>
        </div>
        ...
        {isScanning && (<div className="space-y-2">
            ...
            <progress_1.Progress value={scanProgress} className="h-2"/>
          </div>)}

        {/* Selected Model */}
        {selectedModelData && (<alert_1.Alert className="bg-blue-900/20 border-blue-700">
        ...
        <div className="space-y-2">
          <label_1.Label className="text-sm text-gray-300">Detected Models:</label_1.Label>
          <scroll_area_1.ScrollArea className="h-64">
            <div className="space-y-2">
              {models.map((model) => (<div key={model.id} className={`border rounded p-3 space-y-2 ${selectedModel === model.id
                ...
                : "border-slate-600"}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {model.bengaliSupport && (<badge_1.Badge variant="outline" className="text-xs border-orange-600 text-orange-300">
                          🇧🇩 Bengali
                        </badge_1.Badge>)}
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <progress_1.Progress value={model.performance.speed} className="h-1"/>
                    
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-400">Accuracy</span>
                      <span className="text-gray-300">{model.performance.accuracy}%</span>
                    </div>
                    <progress_1.Progress value={model.performance.accuracy} className="h-1"/>
                  </div>

                  {/* Capabilities */}
                  <div className="flex flex-wrap gap-1">
                    {model.capabilities.map((capability) => (<badge_1.Badge key={capability} variant="outline" className="text-xs border-slate-600 text-slate-300">
                        {capability}
                      </badge_1.Badge>))}
                  </div>
                  ...
                  <div className="flex gap-2">
                    {model.status === "available" ? (selectedModel !== model.id && (<button_1.Button size="sm" onClick={() => setSelectedModel(model.id)} className="h-6 text-xs bg-blue-600 hover:bg-blue-700">
                          <lucide_react_1.Zap className="h-3 w-3 mr-1"/>
                          Select
                        </button_1.Button>)) : (<button_1.Button size="sm" onClick={() => loadModel(model.id)} disabled={model.status === "loading"} className="h-6 text-xs bg-green-600 hover:bg-green-700">
                        {model.status === "loading" ? (<lucide_react_1.RefreshCw className="h-3 w-3 mr-1 animate-spin"/>) : (<lucide_react_1.Brain className="h-3 w-3 mr-1"/>)}
                        Load
                      </button_1.Button>)}
                  </div>
                </div>))}
        ...
        {/* Status Alert */}
        <alert_1.Alert className={`${availableModels.length > 0
